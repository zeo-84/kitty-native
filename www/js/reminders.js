...
function cdHTML(msLeft) {
  ...
  return `
    ...
    <div class="cd-box"><b>${toPersianIfFa(h)}</b><span>${
    s.cdHours
  }</span></div>
    <div class="cd-box"><b>${toPersianIfFa(m)}</b><span>${s.cdMin}</span></div>
    <div class="cd-box"><b>${toPersianIfFa(sec)}</b><span>${
    s.cdSec
  }</span></div>`;
}
...
function showDueModal() {
  ...
  overlay.innerHTML = `
    <div class="rem-modal">
      <img src="images/kitty_reminder.webp" alt="kitty">
      <h2>${s.dueHello(state.userName || "")}</h2>
      <p class="rem-modal-title">«${escapeHtml(r.title)}»</p>
      ${r.desc ? `<p class="rem-modal-desc">${escapeHtml(r.desc)}</p>` : ""}
      <p class="rem-modal-fun">${s.dueFun}</p>
      <button class="btn-primary" onclick="dismissDue()">${s.okBtn}</button>
    </div>`;
...
function remEditorHTML() {
  ...
  for (let d = 1; d <= len; d++) {
    const sel =
      remDraft.jd === d && remDraft.jm === remCal.m && remDraft.jy === remCal.y;
    cells += `<button class="cal-day ${
      sel ? "sel" : ""
    }" onclick="remSelectDay(${d})">${toPersianIfFa(d)}</button>`;
  }

  return `
    <h2 class="section-title">${
      remDraft.editId ? s.editReminder : s.newReminder
    }</h2>
    <div class="card">
      ...
      <input id="rem-title" class="text-input" style="margin:0;text-align:start"
             placeholder="${s.remTitlePh}" value="${escapeAttr(
    remDraft.title
  )}" oninput="remDraft.title=this.value">
      <textarea id="rem-desc" class="note-textarea" style="min-height:90px"
                placeholder="${
                  s.remDescPh
                }" oninput="remDraft.desc=this.value">${escapeHtml(
    remDraft.desc
  )}</textarea>

      <p style="margin:10px 0 8px;font-weight:bold">${s.remDate}</p>
      <div class="cal-head">
        <button class="btn-secondary" style="margin:0;padding:6px 12px;font-size:13px" onclick="remCalPrev()">${
          s.prev
        }</button>
        <b>${JALALI_MONTHS[remCal.m - 1]} ${toPersianIfFa(remCal.y)}</b>
        <button class="btn-secondary" style="margin:0;padding:6px 12px;font-size:13px" onclick="remCalNext()">${
          s.next
        }</button>
      </div>
      ...
      <input id="rem-time" type="time" class="time-input" value="${
        remDraft.time
      }">

      <div class="editor-actions">
        <button class="btn-primary" style="margin:0;flex:1" onclick="saveReminder()">${
          s.saveReminder
        }</button>
        <button class="btn-secondary" style="margin:0" onclick="remClose()">${
          s.cancel
        }</button>
      </div>
    </div>`;
  ...
  const list = rems.length
    ? rems
        .map((r) => {
          const dt = new Date(r.at);
          const dateStr =
            dt.toLocaleDateString(state.locale === "fa" ? "fa-IR" : "en-US") +
            " — " +
            toPersianIfFa(
              String(dt.getHours()).padStart(2, "0") +
                ":" +
    ...
    <div class="card note-card">
      <div class="note-head">
        <b class="note-title">${icon(
          "reminders",
          18,
          "var(--accent)"
        )} ${escapeHtml(r.title)}</b>
        <div class="note-actions">
          <button class="icon-btn" onclick="openEditReminder(${r.id})">${icon(
            "edit",
            18,
            "var(--accent)"
          )}</button>
          <button class="icon-btn" onclick="deleteReminder(${r.id})">${icon(
            "delete",
            18,
            "#e53935"
          )}</button>
   ...
   return `
    ...
    <div class="search-row">
      <div class="search-box">${icon(
        ...
        20,
        "var(--accent)"
      )}<span>${countText}</span></div>
      <button class="add-btn" onclick="openNewReminder()">${icon(
        "add",
        24,
        "#fff"
      )}</button>
...
// تایمر سراسری — هر ثانیه چک می‌کنه
